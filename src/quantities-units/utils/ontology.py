...
def split_prefixed_applicable_units(
    applicable_units_str=None, prefix_name_list=None, debug=False
):
...
def get_unit_dict(quantity_kind=None, prefix_name_list=None):
    """Function to extract units as dictionary from quantity kind."""
    # Check that parameters are provided
    if quantity_kind is None or prefix_name_list is None:
        exception_message = "Please provide all the parameters."
        raise Exception(exception_message)
    unit_dict = {}
    all_prefixed_units = []
    all_non_prefixed_units = []
    quant_kind_list = quantity_kind["results"]["bindings"]
    all_non_prefixed_units = []
...
def get_main_string(unit_str=None, prefix_name_list=None):
    # Check that parameters are provided
...
def get_unit_prefix(unit_str=None, prefix_name_list=None):
...
# TODO: Add prefix_name_list as param
...
def get_osw_quantity_unit_obj_list(
    prefixes_json=None,
    prefix_name_list=None,
    qudt_units=None,
    qudt_quantity_kind=None,
):
    """Function to extract the QuantityUnit objects from the QUDT API."""
    i = 0
    units = []
    unit_dict = get_unit_dict(
        quantity_kind=qudt_quantity_kind,
        prefix_name_list=prefix_name_list,
    )
    ...
    for non_prefixed_unit_iri, unit_property_dict in unit_dict.items():
        ...
        match_unit_dict = match_object_json_path(
            qudt_units_param_res=qudt_units,
...
if __name__ == "__main__":
    ...
    # Transform data
    osw_quanity_unit_obj_list = get_osw_quantity_unit_obj_list(
